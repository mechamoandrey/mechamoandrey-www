...
// Distributes n items along an arc from startAngle, sweeping by `arc` radians.
// Negative arc sweeps clockwise in CSS coords (y-down).
const pointOnCircle = (
  i: number,
  n: number,
  r: number,
  ...
  const theta = n <= 1 ? startAngle : startAngle + arc * (i / (n - 1));
...
const MenuTrigger = ({
  setIsOpen, isOpen, itemsLength, closeAnimationCallback, openIcon, closeIcon
}: MenuTriggerProps) => {
