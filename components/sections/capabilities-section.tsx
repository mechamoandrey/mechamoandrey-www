...
/* ─── Theme-aware accent ──────────────────────────────────────────────────────
   Each capability has two accent hexes:
   - `accent`     → vibrant tone for bg fills, borders, glows (works on dark)
   - `accentText` → darkened variant used wherever the accent is the FOREGROUND
                    of small text. Required for WCAG AA on light cream bg.
   `useAccent()` returns the right one for the active theme. */
function useAccent(cap: { accent: string; accentText: string }) {
  const { resolvedTheme } = useTheme();
  return resolvedTheme === "light" ? cap.accentText : cap.accent;
}
...
const CAPABILITIES = [
  {
    ...
    accent:     "#c07840",  /* vibrant for dark mode & bg fills */
    accentText: "#8A5024",  /* darkened — 5.9:1 on cream */
  },
  {
    number: "02",
    ...
    accent:     "#c9a86a",
    accentText: "#7E5F26",  /* deep bronze — 6.0:1 on cream */
  },
  {
    number: "03",
    ...
    accent:     "#7A8B5C",
    accentText: "#4E5D32",  /* deep sage — 6.8:1 on cream */
  },
  {
    number: "04",
    ...
    accent:     "#9c7558",
    accentText: "#6E4A30",  /* deep brown — 6.3:1 on cream */
  },
] as const;

/* ─── TagList ────────────────────────────────────────────────────────────────── */
function TagList({ tags, accent, accentText, max }: {
  tags: readonly string[]; accent: string; accentText: string; max?: number;
}) {
      ...
      {list.map((tag) => {
        ...
        return (
          <motion.span
            ...
            style={{ background: `${accent}12`, color: accentText, border: `1px solid ${accent}28` }}
    ...
    >
      <motion.div className="h-full" whileHover={{ y: -4 }}
        ...
        <CardBase accent={cap.accent}>
          ...
          <TagList tags={cap.tags} accent={cap.accent} accentText={accentText} max={3} />
        </CardBase>
      </motion.div>
    </motion.div>
  );
...
/* ════════════════════════════════════════════════════════════════════════════════
   ...
   ════════════════════════════════════════════════════════════════════════════════ */
function MiniGauge({ accent, accentText, active }: { accent: string; accentText: string; active: boolean }) {
    ...
    <div className="flex items-center gap-4">
      <div className="relative shrink-0">
        <svg viewBox="0 0 64 64" className="w-16 h-16 -rotate-90">
          <circle cx="32" cy="32" r={r} fill="none" stroke="color-mix(in oklch, var(--foreground) 8%, transparent)" strokeWidth="5" />
        ...
        <div className="absolute inset-0 flex items-center justify-center">
          <span className="text-sm font-bold font-mono" style={{ color: accentText }}>{score}</span>
        </div>
      </div>
      <div>
        ...
        <div className="mt-2 flex flex-col gap-1">
          {[["LCP", "1.1s"], ["INP", "58ms"], ["CLS", "0.01"]].map(([label, val]) => (
            <div key={label} className="flex items-center gap-2">
              ...
              <motion.span className="text-[11px] font-mono font-bold" style={{ color: accentText }}
    ...
    >
      <motion.div className="h-full" whileHover={{ y: -4 }}
        ...
        <CardBase accent={cap.accent}>
          ...
          <h3 className="text-2xl md:text-3xl font-bold tracking-tight leading-tight text-foreground">{cap.title}</h3>
          <p className="text-[11px] font-semibold uppercase tracking-widest" style={{ color: accentText }}>{cap.subtitle}</p>
          <MiniGauge accent={cap.accent} accentText={accentText} active={active} />
...
/* ════════════════════════════════════════════════════════════════════════════════
   ...
   ════════════════════════════════════════════════════════════════════════════════ */
function SimplifiedRow({ cap, i }: { cap: typeof CAPABILITIES[number]; i: number }) {
  const accentText = useAccent(cap);
  return (
    <motion.div
    ...
    >
      <div className="flex flex-col justify-between gap-6">
        <div>
          <span className="text-[11px] font-mono font-bold tracking-widest mb-4 block" style={{ color: accentText }}>{cap.number}</span>
          {cap.number === "03"
            ? <BlurText text={cap.title} as="h3" animateBy="words" direction="top"
                delay={110} stepDuration={0.38}
      ...
      <div className="flex flex-col gap-5">
        <p className="text-xs font-semibold uppercase tracking-widest" style={{ color: accentText }}>{cap.subtitle}</p>
    ...
    <section id="capacidades" className="px-6 md:px-12 lg:px-20 py-28 md:py-36 max-w-[1400px] mx-auto">
      ...
      <div className="flex flex-col">
        {CAPABILITIES.map((cap, i) => (
          <SimplifiedRow key={cap.number} cap={cap} i={i} />
        ))}
